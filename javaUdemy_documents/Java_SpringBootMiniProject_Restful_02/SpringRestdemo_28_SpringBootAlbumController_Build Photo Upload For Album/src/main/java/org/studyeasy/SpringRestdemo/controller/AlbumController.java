    ...
    public ResponseEntity<List<String>> photos(@RequestPart(required = true) MultipartFile[] files,
        ...
        Arrays.asList(files).stream().forEach(file -> {
            ...
            || contentType.equals("image/jpeg")){
                ...
                // When we are storing the file in the database, there is a possibility that file name from the user
                // is repeating and if that happens, then file from the user will get repeated and in server will
                // get replaced. 
